...
//        onView(withId(R.id.navigation_contact)).perform(click());
        mActivityRule.getActivity().getSupportFragmentManager().beginTransaction().replace(R.id.content, ContactFragment.newInstance()).commit();
...
//        onView(withText(R.id.listViewTopArticle)).check(doesNotExist());
    ...
    public void loadUserData() throws Exception {
        ...
        onView(new RecyclerViewMatcher(R.id.list).atPositionOnView(9, R.id.textViewUsername)).check(matches(withText("mixik")));
